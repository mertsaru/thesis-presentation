...
\begin{document}
	...
	\begin{frame}[fragile]{Optimizers}{Loose Symmetry (LS)}
		...
		\item <3-> Has better results than other cognitive bias optimizers~\cite{smt} %TODO: add citation
		\end{itemize}
	\end{frame}
	...
	\begin{frame}[fragile]{Optimizers}{Loose Symmetry under Rarity (LSR)}
		...
		\vspace{10pt}
		\item<5-> Satisfies ?~\cite{smt} %TODO: add citation
		\end{itemize}
	\end{frame}
	\section{Results}
	\begin{frame}[fragile]{Results}{Balanced dataset}
		\scriptsize
		Dataset: Ionosphere dataset~\cite{ion} %TODO: add citation
		\newline
...
\centering		\color{red}{Be careful with y-axis, since they do not share common size} %TODO add color red
	
	\end{frame}
	
	
	% \begin{frame}[fragile]{Results}{Balanced dataset}
	% 	\scriptsize Dataset: ? Ionosphere~\cite{dataset}
	% 	\begin{columns}
	...
	\makebibliography % TODO create bib
