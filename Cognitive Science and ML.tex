...
\begin{document}
	...
	\begin{frame}[fragile]{ML Implementation}{Interpretation}
		\begin{columns}
			...
			\begin{column}[T]{.15\textwidth}
				\uncover<3->{
					\begin{tikzpicture}
						\draw[->, line width=3mm] (8,-10) -- (9,-10); % TODO Resize arrow
						\end{tikzpicture}
				}
			\end{column}
	...
	\begin{frame}[fragile]{Optimizers}{Loose Symmetry (LS)}
		...
		+ \frac{b_{i}(t)}{b_{i}(t) + d_{i}(t)}d_{i}(t) 
		+ b_{i}(t) 
		+ \frac{a_{i}(t)}{a_{i}(t) + c_{i}(t)}c_{i}(t)}$~\cite{3, 7} %TODO: add citation
		\vspace{10pt}
		\item<2-> Satisfies XM and loosely satisfies S, MX, and ER~\cite{3} %TODO: add citation
		\item <3-> Has better results than other cognitive bias optimizers~\cite{something} %TODO: add citation
		\end{itemize}
	\end{frame}
	...
	\begin{frame}[fragile]{Optimizers}{Loose Symmetry under Rarity (LSR)}
		...
		\item<1-> Assumption: The events $p$ and $q$ are small, hence the correlation of any two events is unlikely, $d(t) \rightarrow \infty$~\cite{22} %TODO: add citation
		\item <2-> Example: The correlation between any random event and you starting you car in the morning~\cite{22} %TODO: add citation
		\item<3-> $R^{\text{LSR}}_{i}(t) = \lim_{d_{i}(t) \to \infty}R^{\text{LS}}_{i}(t)$
		\item<4-> $R^{\text{LS}}_{i}(t) = \frac{a_{i}(t) 
		+ b_{i}(t)}
		{a_{i}(t) 
		+ 2b_{i}(t) 
		+ \frac{a_{i}(t)}{a_{i}(t) + c_{i}(t)}c_{i}(t)}$~\cite{3} %TODO: add citation
		\vspace{10pt}
		\item<5-> Satisfies ?~\cite{something} %TODO: add citation
		\end{itemize}
	\end{frame}
	...
	% 	Dataset: ? dataset~\cite{dataset}
	% 	\begin{columns}
			
	% 		\begin{column}[T]{0.33\textwidth}
	% 			\begin{figure}
	% 				\includegraphics[width=0.9\textwidth]{myfigs/LS.png}
	% 				\caption{LS~\cite{something}}
	% 			\end{figure}
	% 		\end{column}
